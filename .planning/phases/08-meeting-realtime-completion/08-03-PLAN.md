...
First, add `import asyncio` at the top of main.py alongside the other standard library imports (it is NOT currently imported there). Use the bare `asyncio` name -- do NOT use an alias.
...
IMPORTANT: Use `asyncio.CancelledError` directly -- asyncio is imported at top level in Step 2. Do NOT use any alias.
  </action>
  <verify>
    grep -n "import asyncio" src/app/main.py
    ...
    grep -n "asyncio.CancelledError" src/app/main.py
  ...
  <done>
    CalendarMonitor.run_poll_loop() is started as an asyncio background task in main.py lifespan using bare `asyncio` (no alias). POLL_INTERVAL_SECONDS updated from 60 to 900 (15 min). Shutdown section cancels the monitor task gracefully using `asyncio.CancelledError` (calls .stop() then .cancel()). Active pipelines cleaned up on shutdown.
  </done>
</task>
...
7. **test_websocket_finds_pipeline_on_app_state**
   ...
   - This test also verifies UUID string format roundtrip: the meeting_id used as the WebSocket path parameter (a string) matches the format produced by str(uuid.UUID) which is what _build_output_media_url passes as the query param and what setattr uses for app.state key naming.
...
Recommended: For tests 1-3, patch `_create_pipeline_for_meeting` to return a MagicMock pipeline. For test 4, let it run but patch the component constructors. This keeps tests fast and focused.
  ...
  <done>
    Integration tests verify: (1) pipeline created on in_call_recording, (2) pipeline cleaned up on call_ended, (3) pipeline skipped without API keys, (4) pipeline has correct meeting context, (5) POLL_INTERVAL_SECONDS is 900, (6) run_poll_loop calls process_upcoming_meetings, (7) WebSocket finds pipeline on app.state (also verifies UUID string format roundtrip between _build_output_media_url and WebSocket path param). All existing tests continue to pass.
  </done>
</task>
...
4. `grep "asyncio.CancelledError" src/app/main.py` -- Uses bare asyncio (no alias)
5. `python -m pytest tests/test_meeting_realtime_wiring.py -v --timeout=30` -- All new tests pass
6. `python -m pytest tests/ -x --timeout=120 -q` -- Full suite passes (1116+ tests)
7. `python -c "from src.app.main import create_app; print('OK')"` -- App creates without error
</verification>

<success_criteria>
- CalendarMonitor.run_poll_loop() started as asyncio background task in main.py
- POLL_INTERVAL_SECONDS is 900 (15 minutes)
- Shutdown cancels calendar_monitor_task and cleans up active pipelines using bare asyncio (no alias)
- Integration tests pass covering: pipeline lifecycle (create/cleanup), WebSocket discovery (with UUID roundtrip verification), calendar monitor interval
