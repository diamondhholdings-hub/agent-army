...
must_haves:
  truths:
    ...
    - "SA routing rules direct all 5 task types to solution_architect agent_id"
...
<objective>
Wire the Solution Architect agent into the application: register it in main.py lifespan, add routing rules, and create integration tests that verify all 5 capabilities work end-to-end (with mocked LLM).

Purpose: This is the final integration step that makes the SA agent discoverable, routable, and invocable within the running application. Without this wiring, the agent exists but cannot be reached. Tests validate the entire chain: task input -> handler routing -> LLM call -> response parsing -> typed output.
...
<task type="auto">
  ...
  <done>
    10 tests pass covering: all 5 task type handlers, unknown type error, fail-open behavior, registration correctness, handoff payload construction, and content type validation. All tests are self-contained with mocked dependencies.
  </done>
</task>
...
<verification>
1. `python -m pytest tests/test_solution_architect.py -v --timeout=30` -- all 10 tests pass
...
- 10 integration tests pass covering all 5 capabilities + error handling + registration
