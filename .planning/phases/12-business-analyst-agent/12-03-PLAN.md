...
must_haves:
  truths:
    ...
    - "create_requirements_page returns page_id (UUID), matching NotionPMAdapter pattern"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/agents/business_analyst/notion_ba.py"
      ...
      via: "follows same return-page_id pattern"
      pattern: 'page\.get\("id"'
---
...
1. `async def create_requirements_page(self, deal_id: str, result: BAResult, title: str | None = None) -> str`:
   ...
   - **Returns page_id (the Notion page UUID)** -- use `page.get("id", "")` to match the established NotionPMAdapter pattern. Do NOT return `page.get("url", "")`.
...
The Notion API call pattern (same as notion_pm.py, but returning page_id not URL):
```python
...
Run: `cd "/Users/RAZER/Documents/projects/sales army" && grep -n 'page.get' src/app/agents/business_analyst/notion_ba.py` -- must show `page.get("id"` (NOT `page.get("url"`).
  </verify>
  <done>
NotionBAAdapter with 3 retry-wrapped CRUD methods. Takes pre-authenticated AsyncClient. create_requirements_page combines all 4 renderers based on BAResult content and returns page_id (UUID), matching NotionPMAdapter pattern.
  </done>
</task>
...
4. `grep -n 'page.get' src/app/agents/business_analyst/notion_ba.py` confirms `page.get("id"` (not "url")
