...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/main.py"
      to: "app.state.business_analyst"
      via: "state assignment for runtime access"
      pattern: "app\\.state\\.business_analyst"
...
Run: `cd "/Users/RAZER/Documents/projects/sales army" && grep -n 'phase12.business_analyst' src/app/main.py` -- must find the log line.

Run: `cd "/Users/RAZER/Documents/projects/sales army" && grep -n 'app.state.business_analyst' src/app/main.py` -- must find the state assignment line, confirming BA agent is stored on app.state for runtime access.
  </verify>
  <done>
BA agent initialization block added to main.py lifespan, positioned after PM and before Deal Management. Follows identical pattern to SA/PM wiring. app.state.business_analyst is set.
  </done>
</task>
...
3. `grep 'app.state.business_analyst' src/app/main.py` -- state assignment exists
...
- app.state.business_analyst is set during startup
