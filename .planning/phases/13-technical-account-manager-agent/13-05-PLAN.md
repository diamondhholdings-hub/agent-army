...
must_haves:
  truths:
    ...
    - "Co-dev opportunity events from TAM are acknowledged by Sales Agent"
  artifacts:
...
Purpose: Completes the TAM agent's integration into the multi-agent system. Sales Agent can trigger health checks, escalation outreach, and other TAM tasks from conversations. TAM co-dev opportunities flow back to Sales Agent via event bus.
