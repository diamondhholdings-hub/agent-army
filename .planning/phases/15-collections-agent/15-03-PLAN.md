...
must_haves:
  truths:
    ...
    - "handle_run_escalation_check calls handle_generate_collection_message internally after advancing to stages 1-4, producing a Gmail draft before returning"
    - "build_escalation_check_prompt is used to generate the Stage 5 notification email body (for both rep draft and finance team draft) before calling gmail_service.create_draft()"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/agents/collections/handlers.py"
      ...
      via: "build_* functions called per task type, including build_escalation_check_prompt for stage 5 notification body"
    ...
    - from: "src/app/agents/collections/handlers.handle_run_escalation_check"
      to: "src/app/agents/collections/handlers.handle_generate_collection_message"
      via: "internal call after stage advancement to produce Gmail draft for stages 1-4"
      pattern: "handle_generate_collection_message"
---
...
Purpose: Handlers are the core logic layer between the agent supervisor and external services. Each handler implements one collections capability. Fail-open semantics keep the workflow unblocked under LLM errors. Escalation advancement (stages 1-4) immediately produces a Gmail draft so reps always have a draft ready when a stage advances. Stage 5 uses build_escalation_check_prompt to generate notification body for both the rep and finance team drafts.
...
Builds prompt to generate the Stage 5 human-handoff notification content. This prompt is used ONLY when generating the stage 5 notification email body (for both the rep draft and the finance team draft). It should produce a clear, professional summary explaining why the account has reached human handoff, the AR history, and recommended next steps.
Embeds schema: `{"subject": str, "body": str, "summary_for_finance": str, "key_facts": list[str]}`.
NOTE: Handler does NOT use LLM for escalation stage advancement decisions (those are deterministic). This builder is used exclusively to generate the Stage 5 notification email content.
...
Export `__all__` listing all 5 builders + COLLECTIONS_SYSTEM_PROMPT.
  ...
  <verify>
Run: `cd /Users/RAZER/Documents/projects/sales\ army && python -c "from src.app.agents.collections.prompt_builders import COLLECTIONS_SYSTEM_PROMPT, build_ar_report_prompt, build_risk_narrative_prompt, build_collection_message_prompt, build_escalation_check_prompt, build_payment_plan_prompt; p = build_collection_message_prompt('acct1', 2, {'total_outstanding_usd': 5000, 'oldest_invoice_number': 'INV-001'}, 0.9, {}); assert 'INV-001' in p or 'oldest_invoice' in p; p2 = build_escalation_check_prompt('acct1', {'current_stage': 5}, {}, {}); assert len(p2) > 50; print('prompt builders OK')"`
  </verify>
  <done>All 5 prompt builders importable, return non-empty strings, include embedded JSON schema. COLLECTIONS_SYSTEM_PROMPT defined. build_escalation_check_prompt returns a stage 5 notification content prompt (not a decision/advancement prompt).</done>
</task>

<task type="auto">
  <name>Task 2: Create 5 task handlers with fail-open semantics and draft-on-advance for stages 1-4</name>
...
NOTE: This handler does NOT call csm_agent — the agent's _execute_task method (in agent.py) checks the returned result's rag field and calls self.receive_collections_risk() directly. Handlers have no csm_agent reference.

**handle_generate_collection_message(task, llm_service, notion_collections, gmail_service, scorer, **kwargs) -> dict**:
1. Extract account_id, stage (1-4 only — stage 5 is human handoff, no automated message; return error if stage==5), ar_report data, account_context
...
5. Apply DETERMINISTIC escalation logic (NOT LLM) using STAGE_TIME_FLOORS:
   - If current_stage == 5: already at human handoff, skip and return current state
   ...
   - If BOTH conditions met: advance stage (current_stage + 1)
   - If new stage is 1-4: call `await handle_generate_collection_message(...)` internally with the new stage to produce a Gmail draft for the rep. Pass the same llm_service, notion_collections, gmail_service, scorer, and current ar_report. This ensures every stage advancement produces a ready-to-send draft. Wrap in try/except (fail-open: draft failure does not block stage advancement).
   - If advancing to stage 5: set stage5_notified=False (trigger notification). Then generate stage 5 notification body using `build_escalation_check_prompt(account_id, escalation_state_dict, ar_report_dict, risk_result_dict)` + LLM call. Parse `subject`, `body`, and `summary_for_finance` from LLM response. Create rep draft via `await gmail_service.create_draft(subject=subject, body=body)`. Create finance team draft via `await gmail_service.create_draft(subject=f"[Finance Handoff] {subject}", body=summary_for_finance, to=finance_team_email)`. Set stage5_notified=True. Wrap all draft creation in try/except (fail-open).
6. Update escalation_state in notion_collections
7. Return CollectionsAlertResult model_dump()

**CRITICAL CONSTRAINT**: Handlers have no csm_agent reference. The CSM notification path is handled entirely by CollectionsAgent._execute_task() in agent.py (which checks result["rag"] after payment_risk_assessment and calls self.receive_collections_risk). Do NOT add csm_agent to handler signatures.
...
Import `STAGE_TIME_FLOORS` from scorer module. Import `compute_tone_modifier` from scorer module. Import `build_escalation_check_prompt`, `build_collection_message_prompt` from prompt_builders. Use lazy imports where needed to avoid circular deps (same pattern as other agents).

For finance_team_email in stage 5: get it from `task.get("finance_team_email", "")` or from kwargs. If empty string, skip finance draft and log warning — do not raise.
  </action>
  <verify>
...
Also verify: `cd /Users/RAZER/Documents/projects/sales\ army && python -c "from src.app.agents.collections.handlers import handle_run_escalation_check; import inspect; src = inspect.getsource(handle_run_escalation_check); assert 'handle_generate_collection_message' in src; assert 'build_escalation_check_prompt' in src; print('escalation handler wiring OK')"`
  </verify>
  <done>All 5 handlers importable, callable with None services, return dict with either valid data or {"error", "confidence", "partial"} fail-open response. handle_run_escalation_check calls handle_generate_collection_message internally after advancing to stages 1-4. handle_run_escalation_check uses build_escalation_check_prompt to generate Stage 5 notification body. compute_tone_modifier imported from scorer. STAGE_TIME_FLOORS imported from scorer. No csm_agent reference in any handler signature.</done>
</task>
...
3. Grep for `create_draft` in handlers.py — message generation, stage 5 rep draft, and stage 5 finance draft all use gmail_service.create_draft(), never send_email
4. Grep for `FINANCE_TEAM_EMAIL\|finance.*email\|finance.*draft\|finance.*notify` in handlers.py — stage 5 human handoff notifies finance team
5. Grep for `handle_generate_collection_message` in handlers.py — appears inside handle_run_escalation_check (internal call for stages 1-4)
6. Grep for `build_escalation_check_prompt` in handlers.py — used inside handle_run_escalation_check for stage 5 notification body
</verification>

<success_criteria>
- 5 prompt builders in prompt_builders.py, all returning str, COLLECTIONS_SYSTEM_PROMPT defined
- build_escalation_check_prompt generates stage 5 notification content (not an advancement decision prompt)
...
- After advancing to stages 1-4: handle_generate_collection_message called internally to produce Gmail draft
- Stage 5 uses build_escalation_check_prompt + LLM to generate notification body, then triggers Gmail drafts to both rep AND finance team
- All communications via create_draft(), never send_email()
- No csm_agent reference in any handler (CSM notification is the agent's responsibility, not the handler's)
