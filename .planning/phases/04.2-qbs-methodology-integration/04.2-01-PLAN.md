...
**Note on anti-interrogation enforcement:** The `QBS_METHODOLOGY_PROMPT` includes the rule "Never ask more than one probing question per message." This is enforced at the prompt instruction level. As a future improvement (post-MVP), consider adding optional post-generation validation in the SalesAgent pipeline: after the LLM generates a message, count question marks using a simple regex pattern (e.g., `len(re.findall(r'\?', generated_text))`), and log a warning if more than one question is detected. This could be implemented as a lightweight validation step in `_handle_send_email()` / `_handle_send_chat()` in Plan 03's agent.py integration. Not required for MVP -- the prompt instruction is the primary enforcement mechanism.
  </action>
