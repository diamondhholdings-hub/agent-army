...
must_haves:
  truths:
    ...
    - "SalesAgent._get_qbs_guidance() runs QBS engine analysis before message generation and returns dynamic prompt section"
    - "SalesAgent._handle_send_email() and _handle_send_chat() call _get_qbs_guidance() to inject QBS guidance into prompts before LLM call"
  ...
  key_links:
    - from: "src/app/agents/sales/agent.py"
      ...
      via: "QBSQuestionEngine injected in constructor and called in _get_qbs_guidance"
