...
<task type="auto">
  ...
  <action>
    ...
    3. Create `src/app/meetings/bot/manager.py` with `BotManager`:
       - Constructor takes: `recall_client: RecallClient`, `repository: MeetingRepository`, `settings` (for webapp URL, bot name)
  ...
  <done>
    RecallClient provides async Recall.ai REST API wrapper with retry logic. BotManager handles full bot lifecycle: creation with Output Media config, early joining, status tracking, webhook event processing, and post-meeting artifact retrieval.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    5. Create `tests/test_bot_services.py` with unit tests:
       ...
       - Minimum 12 tests covering all service wrappers
  </action>
  <verify>
...
<success_criteria>
All external API service wrappers are built and tested -- RecallClient/BotManager for meeting bot lifecycle, DeepgramSTT for streaming transcription, ElevenLabsTTS for streaming voice synthesis, HeyGenAvatar for animated avatar with lip-sync. These are ready for orchestration in Plan 06-04.
</success_criteria>
