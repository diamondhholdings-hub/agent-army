...
must_haves:
  truths:
    ...
    - "SilenceChecker enforces all three strategic silence rules: not during thinking pause, not when internal rep speaking, not when confidence below threshold"
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `src/app/meetings/realtime/silence_checker.py` with `SilenceChecker`:
       ...
       - `async should_respond(transcript: str, speaker_id: str, meeting_context: dict) -> bool`:
         Implements ALL THREE strategic silence rules from CONTEXT.md:
         ...
         c. This method returns True only if conditions (a) and (b) pass
            - Confidence check (c) is performed later during LLM generation, not here
            - Document this architectural choice: confidence is assessed as part of LLM response generation
    ...
    3. Create `src/app/meetings/realtime/pipeline.py` with `RealtimePipeline` and `PipelineMetrics`:
       ...
       - `async process_speech_turn(final_transcript: str, speaker_id: str) -> None`:
         ...
         2. Check silence_checker.should_respond -- if False, log and return
         ...
         5. Buffer LLM tokens to sentence boundaries for TTS quality
         6. At each sentence boundary: send text to avatar.speak() for lip-synced delivery
         7. Record metrics at each stage
         8. Log total_latency_ms via structlog
       ...
       - `_build_llm_context(current_transcript: str) -> list[dict]`:
         ...
         - Include instruction: "If confidence is low, respond with SILENCE_TOKEN instead of speaking"
         - SILENCE_TOKEN = "[SILENCE]" -- if LLM returns this, pipeline stays silent (confidence check)
    ...
    4. Create `tests/test_realtime_pipeline.py` with unit tests:
       ...
       - Test RealtimePipeline SILENCE_TOKEN handling (LLM says don't speak)
  ...
  <done>
    TurnDetector distinguishes pause types. SilenceChecker enforces all strategic silence rules. RealtimePipeline orchestrates STT->LLM->TTS with sub-1s latency target, sentence-boundary buffering, LLM timeout handling, and SILENCE_TOKEN confidence gating. All unit tests pass.
  </done>
</task>
