...
must_haves:
  truths:
    - "Meeting API endpoints provide CRUD for meetings, briefings, minutes, bot status, and webhook handling"
    ...
    - "CalendarMonitor poll loop starts as background task on application startup"
    - "Integration tests verify end-to-end flows: briefing generation, bot creation, minutes generation"
  artifacts:
