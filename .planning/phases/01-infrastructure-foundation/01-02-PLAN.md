...
files_modified:
  ...
  - src/app/services/google_workspace.py
...
user_setup:
  ...
  - service: google_workspace
    why: "Domain-wide delegation for Gmail, Calendar, Meet access"
    env_vars:
      - name: GOOGLE_SERVICE_ACCOUNT_FILE
        source: "GCP Console -> IAM -> Service Accounts -> Create Key (JSON) (https://console.cloud.google.com/iam-admin/serviceaccounts)"
    dashboard_config:
      - task: "Enable domain-wide delegation for the service account"
        location: "Google Workspace Admin -> Security -> API Controls -> Domain-wide Delegation (https://admin.google.com/ac/owl/domainwidedelegation)"
      - task: "Add OAuth scopes: gmail.readonly, gmail.send, calendar, calendar.readonly"
        location: "Same domain-wide delegation page"
...
must_haves:
  truths:
    ...
    - "Google Workspace delegation can impersonate a user in a tenant's domain"
    - "Prompt injection attempts in LLM input are detected and sanitized"
    - "Structured request logging includes tenant_id, user_id, and request duration"
  artifacts:
    ...
    - path: "src/app/services/google_workspace.py"
      provides: "Google Workspace delegation service for Gmail and Calendar"
      contains: "with_subject"
...
<objective>
Build the API gateway layer: JWT authentication with tenant-scoped authorization, LLM provider abstraction via LiteLLM Router (Claude primary, GPT-4o fallback), Google Workspace domain-wide delegation service, prompt injection protection, and structured request logging.
...
Output: Secured API gateway with JWT auth, tenant-scoped authorization, LLM completion endpoint, Google Workspace service (ready for integration), and prompt injection detection. Middleware produces structured logs with tenant and user context.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **9. Create tests/test_auth.py:**
    ...
    - `test_api_key_creation`: Create API key, verify it can be used for authentication.
  </action>
  <verify>
    1. `uv run pytest tests/test_auth.py -v` -- all auth tests pass.
    2. Start server: `uv run uvicorn src.app.main:app --reload`.
    3. Create user via test fixture or direct DB insert.
    4. `curl -X POST http://localhost:8000/api/v1/auth/login -H "X-Tenant-ID: {tenant_id}" -H "Content-Type: application/json" -d '{"email":"test@skyvera.com","password":"test123"}'` returns JWT.
    5. `curl http://localhost:8000/api/v1/auth/me -H "Authorization: Bearer {token}"` returns user info with tenant context.
    6. `curl http://localhost:8000/api/v1/auth/me` (no token) returns 401.
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: LLM provider abstraction, Google Workspace delegation, and prompt injection protection</name>
  <files>
    src/app/services/llm.py
    src/app/services/google_workspace.py
    src/app/schemas/llm.py
  ...
  <action>
    **1. Add LLM config to src/app/config.py:**
    ...
    - `GOOGLE_SERVICE_ACCOUNT_FILE`: str, default "" (path to service account JSON).
    ...
    **3. Create src/app/services/google_workspace.py** -- Google Workspace delegation:

    `GoogleWorkspaceService` class:
    - `__init__(self, service_account_file: str)`: Load credentials from service account JSON.
    - Scopes: `gmail.readonly`, `gmail.send`, `calendar`, `calendar.readonly`.
    - `_get_delegated_credentials(self, user_email: str)`: Create delegated credentials using `credentials.with_subject(user_email)`.
    - `async def get_gmail_service(self, user_email: str)`: Build Gmail API v1 service.
    - `async def get_calendar_service(self, user_email: str)`: Build Calendar API v3 service.
    - `async def list_recent_emails(self, user_email: str, max_results: int = 10) -> list[dict]`: List recent email message IDs.
    - `async def send_email(self, user_email: str, to: str, subject: str, body: str) -> dict`: Send email via domain-wide delegation (impersonating user_email).
    - `async def list_upcoming_events(self, user_email: str, max_results: int = 10) -> list[dict]`: List upcoming calendar events.
    - Note: Actual API calls require the service account to be set up with domain-wide delegation. The code should handle the case where credentials are not configured (return appropriate error, don't crash).

    **4. Create src/app/schemas/llm.py** -- Pydantic models:
    ...
    **5. Create src/app/api/v1/llm.py** -- LLM endpoints:
    ...
    **6. Update router and main.py** to include auth and LLM routers.

    **7. Create tests/test_llm.py:**
    ...
    **8. Create tests/test_security.py:**
  ...
  <verify>
    ...
    4. `python -c "from src.app.services.google_workspace import GoogleWorkspaceService; print('Google Workspace service importable')"` -- module loads without error.
  </verify>
  <done>
    LLM calls route through LiteLLM Router with Claude primary and GPT-4o fallback. Tenant metadata is included in every LLM call for cost tracking. Prompt injection detection catches common patterns and sanitizes input. Google Workspace delegation service is implemented and ready for integration when service account is configured. All auth and LLM tests pass.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    4. Google Workspace delegation service (ready for configuration)
    5. Structured request logging with tenant context
  </what-built>
  <how-to-verify>
    1. Start the server: `cd "/Users/RAZER/Documents/projects/sales training" && uv run uvicorn src.app.main:app --reload`
    2. Open http://localhost:8000/docs to see the OpenAPI documentation with all endpoints
    3. Test auth flow: Create a user, login, get JWT, access protected endpoint
    4. Test LLM completion: Send a message through the /api/v1/llm/completion endpoint (requires ANTHROPIC_API_KEY or OPENAI_API_KEY in .env)
    5. Verify structured logs in terminal show tenant_id and request timing
    6. Run full test suite: `uv run pytest tests/ -v` -- all tests should pass
  </how-to-verify>
...
3. **INF-05 (Google Workspace delegation):** Service implemented with domain-wide delegation pattern. Gmail read/send and Calendar operations. Per-tenant service account credentials. Ready for integration when admin configures delegation.
4. **PLT-10 (Security framework):** Prompt injection detection with heuristic patterns. Input sanitization on all user messages before LLM calls. Architectural defense: tenant data never mixed in LLM context. JWT validation prevents unauthorized access.
</verification>
...
- Google Workspace service imports and initializes without error
