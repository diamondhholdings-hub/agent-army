...
<task type="auto">
  <name>Task 1a: Project scaffold, Docker services, database engine, and tenant context</name>
  <files>
    pyproject.toml
    docker-compose.yml
    src/app/__init__.py
  ...
  <action>
    ...
    **5. Create src/app/core/database.py** -- Async SQLAlchemy engine with tenant isolation:
    ...
    Create `src/app/__init__.py`, `src/app/core/__init__.py`, `src/app/services/__init__.py` as empty files.
  </action>
  <verify>
    ...
    3. `python -c "from src.app.core.tenant import TenantContext, get_current_tenant; print('tenant module OK')"` imports without errors.
    4. `python -c "from src.app.core.database import TenantBase, SharedBase; print('database module OK')"` imports without errors.
  </verify>
  <done>
    Project scaffold with pyproject.toml and docker-compose.yml exists. Dependencies install cleanly. Docker services (PostgreSQL 16, Redis 7) running and healthy. TenantContext propagation via contextvars implemented. Database engine with schema_translate_map and pool checkout reset implemented. SharedBase and TenantBase declarative bases defined.
  </done>
</task>

<task type="auto">
  <name>Task 1b: Redis wrapper, models, API dependencies, and app factory</name>
  <files>
    src/app/core/redis.py
    src/app/models/__init__.py
    src/app/models/shared.py
  ...
  <action>
    **1. Create src/app/core/redis.py** -- Tenant-aware Redis wrapper:
    ...
    **2. Create src/app/models/shared.py** -- Shared schema models:
    ...
    **3. Create src/app/models/tenant.py** -- Per-tenant schema models (initial set):
    ...
    **4. Create src/app/api/deps.py** -- FastAPI dependency injection:
    ...
    **5. Create src/app/main.py** -- FastAPI app factory:
    ...
    Create `src/app/models/__init__.py` and `src/app/api/__init__.py` as empty files.
  </action>
  <verify>
    1. `python -c "from src.app.core.redis import TenantRedis, get_tenant_redis; print('redis module OK')"` imports without errors.
    2. `python -c "from src.app.models.shared import Tenant; print('shared models OK')"` imports without errors.
    3. `python -c "from src.app.models.tenant import User; print('tenant models OK')"` imports without errors.
    4. `python -c "from src.app.main import create_app; app = create_app(); print(app.title)"` prints "Agent Army API".
    5. `python -c "from src.app.api.deps import get_tenant, get_db, get_redis; print('deps OK')"` imports without errors.
  </verify>
  <done>
    TenantRedis with automatic key prefixing implemented. Shared models (Tenant) and tenant models (User) defined with proper schema metadata. FastAPI dependency injection layer wraps database, Redis, and tenant context. App factory creates FastAPI instance with TenantMiddleware and health endpoint. All modules importable and wired together.
  </done>
</task>
