...
must_haves:
  truths:
    ...
    - "NotionPMAdapter can create project records, sub-pages, append status reports, append milestone events, and update earned value metrics via Notion API"
  ...
  artifacts:
    ...
    - path: "src/app/agents/project_manager/notion_pm.py"
      provides: "NotionPMAdapter class with CRUD methods for Projects database and deal sub-pages, including append_milestone_event"
      min_lines: 170
...
Purpose: Prompts drive every LLM-based PM capability (plan generation, risk detection, scope adjustment, report generation). The NotionPMAdapter provides the CRM write layer for project records, sub-pages, milestone events, and metric updates. Together these are the two input-processing modules that the PM agent handlers depend on.
...
8. `async def append_milestone_event(self, page_id: str, milestone_blocks: list[dict]) -> None`
   - Append milestone completion event blocks to the deal sub-page
   - milestone_blocks should contain: milestone name, completion date, success criteria met (bool), notes
   - Uses same `self._client.blocks.children.append()` pattern as append_status_report and append_risk_log_entry
   - Log the milestone event with structlog (include page_id and milestone count)
   - This fulfills the locked decision: "Milestone completion events (recorded)" in CRM writes
...
# Verify append_milestone_event method exists
assert hasattr(NotionPMAdapter, 'append_milestone_event'), 'Missing append_milestone_event method'
print('append_milestone_event method exists')
...
"`
  ...
  <done>NotionPMAdapter with 8 async methods (retry-wrapped), including append_milestone_event for recording milestone completion events. WBS-to-blocks renderer converts 3-level WBS to Notion content. Report-to-blocks renderer converts status reports to Notion blocks.</done>
</task>
...
- NotionPMAdapter importable with all 8 methods (including append_milestone_event)
...
- NotionPMAdapter with retry-wrapped CRUD methods for Projects database, including append_milestone_event
